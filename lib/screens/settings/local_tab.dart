    ...
    try {
      ...
      if (entries.isEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('local_settings.export_no_entries'.tr()),
      ...
      if (storagePath == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('local_settings.export_no_storage'.tr()),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${'local_settings.export_success'.tr([entries.length.toString()])}, ${zipPath.split('/').last}'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('local_settings.export_failed'.tr([e.toString()])),
          backgroundColor: Colors.red,
        ),
      );
    } finally {
                ...
                Row(
                  children: [
                    ...
                    if (settings.logLevel != LogLevel.none)
                      OutlinedButton.icon(
                        onPressed: () async {
                          final path = await logger.exportLogs(widget.currentStoragePath ?? '');
                          if (!context.mounted) return;
                          if (path != null) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('${'local_settings.logs_exported'.tr()} ${path.split('/').last}'),
                                backgroundColor: Colors.green,
                              ),
                            );
                          } else {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('local_settings.no_logs'.tr()),
                                backgroundColor: Colors.orange,
                              ),
                            );
                          }
