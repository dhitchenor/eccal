  ...
  Future<List<CalendarInfo>> listCalendars() async {
    ...
    try {
      ...
      if (!uri.hasScheme || !uri.hasAuthority) {
        logger.debug('CalDAV: Cannot list calendars - invalid URL format: $url');
        return [];
      }
      ...
      if (calendarsIndex != -1 && calendarsIndex + 1 < pathSegments.length) {
        // URL already has calendar path - use the username from path
        final usernameFromPath = pathSegments[calendarsIndex + 1];
        userCalendarsUrl = '${uri.scheme}://${uri.host}${uri.port != 0 && uri.port != 80 && uri.port != 443 ? ':${uri.port}' : ''}/${pathSegments.take(calendarsIndex + 2).join('/')}/';
      } else {
        // Base URL - construct the full path with username
        final basePath = uri.path.endsWith('/') ? uri.path : '${uri.path}/';
        userCalendarsUrl = '${uri.scheme}://${uri.host}${uri.port != 0 && uri.port != 80 && uri.port != 443 ? ':${uri.port}' : ''}${basePath}calendars/$username/';
      }
...
<d:propfind xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  <d:prop>
    ...
    <d:resourcetype/>
      ...
      if (response.statusCode != 207) {
        logger.error('CalDAV: Failed to list calendars: ${response.statusCode}');
      ...
      final displayNameRegex = RegExp(r'<d:displayname>(.*?)</d:displayname>', caseSensitive: false);
      final hrefRegex = RegExp(r'<d:href>(.*?)</d:href>', caseSensitive: false);

      final body = response.body;
      final responses = body.split('<d:response>');

      logger.debug('CalDAV: Found ${responses.length - 1} response blocks in XML');
      ...
      for (final responseBlock in responses.skip(1)) {
        ...
        final hasCalendarType = responseBlock.contains(RegExp(r'<[^>]*:?calendar\s*/>', caseSensitive: false)) ||
                                 responseBlock.contains(RegExp(r'<[^>]*:?calendar>', caseSensitive: false));
        ...
        if (displayNameMatch != null && hrefMatch != null) {
          ...
          logger.debug('CalDAV: Found calendar - name: "$name", id: "$calendarName"');
