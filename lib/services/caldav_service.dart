...
<C:calendar-query xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav">
  ...
  List<CalendarEvent> _parseCalendarEvents(String xmlResponse) {
    ...
    // Extract timezone from VTIMEZONE component if present
    String timezone = 'UTC';
    final timezoneMatch = RegExp(r'BEGIN:VTIMEZONE\s+TZID:([^\s\r\n]+)', multiLine: true).firstMatch(xmlResponse);
    ...
    for (var i = 1; i < eventBlocks.length; i++) {
      ...
      events.add(_parseEventBlock(block, timezone));
    }
    
    return events;
  }

  CalendarEvent _parseEventBlock(String block, String defaultTimezone) {
    String? uid, summary, description, mood, location;
    DateTime? dtstart, dtstamp;
    String timezone = defaultTimezone; // Use timezone from VTIMEZONE
    ...
    for (final line in lines) {
      if (line.startsWith('UID:')) {
        ...
        // Also check for TZID in DTSTAMP
      ...
      } else if (line.startsWith('DTSTART:') || line.contains('DTSTART;')) {
        final parts = line.split(':');
        if (parts.length >= 2) {
          dtstart = _parseDateTime(parts.last.trim());
        }
        // Check for TZID in DTSTART
        if (line.contains('TZID=')) {
          final tzidMatch = RegExp(r'TZID=([^:;]+)').firstMatch(line);
          if (tzidMatch != null) {
            timezone = tzidMatch.group(1)!;
