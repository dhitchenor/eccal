...
class _EntriesSidebarWidgetState extends State<EntriesSidebarWidget> {
                      ...
                      child: InputDecorator(
                        ...
                        child: Text(
                          widget.selectedLocationFilters.isEmpty
                              ...
                              : 'sidebar.selected_count'.tr([widget.selectedLocationFilters.length.toString()]),
  ...
  Widget _buildEntriesList() {
    ...
    if (widget.selectedLocationFilters.isNotEmpty) {
      filteredEntries = filteredEntries.where((entry) {
        ...
        if (entry.location != null && widget.selectedLocationFilters.contains(entry.location)) {
          return true;
        }
  ...
  Widget _buildEntryTile(DiaryEntry entry, {required bool hasLocal, required bool hasServer}) {
    final isSelected = widget.selectedEntry?.id == entry.id;
    
    // DEBUG: Log storage status
    logger.debug('Entry: ${entry.title} (${entry.id}), hasLocal: $hasLocal, hasServer: $hasServer');
    
    return LayoutBuilder(
      builder: (context, constraints) {
        final isMobile = constraints.maxWidth < 300;
        logger.debug('isMobile: $isMobile, maxWidth: ${constraints.maxWidth}');
            ...
            subtitle: Text(
              ...
              style: TextStyle(
                color: Theme.of(context).brightness == Brightness.dark
                    ? Colors.grey.shade200  // Lighter grey for dark theme
                ...
                Builder(
                  builder: (context) {
                    ...
                    if (hasLocal && hasServer) {
                      ...
                      iconType = 'check_circle (both)';
                    } else if (hasServer) {
                      // Server only - blue cloud
                      icon = const Icon(Icons.cloud, size: 20, color: Colors.blue);
                      iconType = 'cloud (server only)';
                    } else if (hasLocal) {
                      // Local only - blue folder
                      icon = const Icon(Icons.folder, size: 20, color: Colors.blue);
                      iconType = 'folder (local only)';
                    } else {
                      // This shouldn't happen, but let's log it
                      icon = const Icon(Icons.error, size: 20, color: Colors.red);
                      iconType = 'error (neither local nor server!)';
                    }             
                    return icon;
                  },
                ),
                SizedBox(width: constraints.maxWidth < 375 ? 2 : 8), // Smaller gap when narrow
  ...
  Future<void> _exportEntry(BuildContext context, DiaryEntry entry, String format) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('sidebar.exporting_as'.tr([entry.title, format])),
          duration: const Duration(seconds: 1),
        ),
      );
      
      // Save entry in requested format using FileStorageService
      // Supports: 'ics', 'txt', 'md'
      final file = await fileStorage.saveEntry(
        entry,
        fileFormat: format, // Use format directly (txt, ics, or md)
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('sidebar.exported_to'.tr([file.path.split('/').last])),
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('sidebar.export_failed'.tr([e.toString()])),
