...
import 'home/caldav_setup_dialog.dart';
import 'home/sidebar_toggle_button.dart';
import 'home/ios_bottom_bar.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  final GlobalKey<EntryEditorWidgetState> _editorKey = GlobalKey<EntryEditorWidgetState>();
  ...
  void initState() {
    ...
    // Configure CalDAV in DiaryProvider and connect settings
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final settings = context.read<SettingsProvider>();
      final provider = context.read<DiaryProvider>();
      
      // Connect settings provider for file format
      provider.setSettingsProvider(settings);
      
      provider.configureCalDAV(
        url: settings.caldavUrl,
        username: settings.caldavUsername,
        password: settings.caldavPassword,
        calendarName: settings.caldavCalendarName,
        eventDurationMinutes: settings.eventDurationMinutes,
      );
      
      // Check if we should show CalDAV setup prompt
      final shouldShowPrompt = settings.showCaldavSetupPrompt && 
                               (settings.caldavUrl == null || settings.caldavUrl!.isEmpty);
      ...
      if (settings.caldavUrl != null && settings.caldavUrl!.isNotEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('home_screen.initializing'.tr()),
            duration: Duration(seconds: 2),
          ),
        );
      }
      
      // Sync with CalDAV on startup (all platforms)
      logger.info('Syncing with CalDAV on startup');
      provider.syncWithCalDAV().then((_) {
        ...
        logger.error('CalDAV sync error: $error');
        // Don't show error snackbar for minor issues
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            Consumer<DiaryProvider>(
              builder: (context, provider, child) {
                ...
                return IconButton(
                  ...
                  onPressed: isPolling ? null : _handleRefresh,
      ...
      endDrawer: _isAndroid ? AndroidDrawer(
        ...
        onAppend: (entryId) {
          final entry = context.read<DiaryProvider>().entries.firstWhere((e) => e.id == entryId);
          setState(() {
  ...
  Future<void> _handleRefresh() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('updates.refreshing'.tr()),
          duration: Duration(seconds: 1),
        ),
      );
    }
    ...
    if (settings.caldavUrl != null && settings.caldavUrl!.isNotEmpty) {
      try {
        ...
        if (errorStr.contains('401') || errorStr.contains('Authentication')) {
          serverError = 'updates.auth_failed'.tr();
        } else if (errorStr.contains('404')) {
          serverError = 'updates.calendar_not_found'.tr();
        } else if (errorStr.contains('Connection') || errorStr.contains('SocketException')) {
          serverError = 'updates.connection_failed'.tr();
        } else if (errorStr.contains('timeout')) {
          serverError = 'updates.timeout'.tr();
        } else {
  ...
  void _deleteEntry({bool deleteFromServer = false, String? entryId}) {
    ...
    try {
      ...
      ScaffoldMessenger.of(scaffoldContext).showSnackBar(
        SnackBar(content: Text('home_screen.entry_notfound'.tr())),
      );
      return;
    }
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(scaffoldContext).showSnackBar(
                    SnackBar(content: Text('delete_dialog.entry_deleted'.tr())),
                  );
                }
              } catch (e) {
                if (mounted) {
                  ScaffoldMessenger.of(scaffoldContext).showSnackBar(
                    SnackBar(content: Text('delete_dialog.error_deleting'.tr([e.toString()]))),
                  );
                }
              }
  ...
  void _handleSaveEntry(String title, String description, String mood, String? location, double? latitude, double? longitude) async {
    final provider = context.read<DiaryProvider>();
    final settings = context.read<SettingsProvider>();
    ...
    try {
      if (_isAppendMode && _selectedEntry != null) {
        ...
        final updatedDescription = '${_selectedEntry!.description}\n\n$appendHeader\n$description';
        ...
        final updatedAppendLocations = [..._selectedEntry!.appendLocations, location ?? ''];
        final updatedAppendLatitudes = [..._selectedEntry!.appendLatitudes, latitude];
        final updatedAppendLongitudes = [..._selectedEntry!.appendLongitudes, longitude];
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('save_dialog.entry_saved'.tr())),
          );
        }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('save_dialog.entry_saved'.tr([e.toString()]))),
        );
      }
    } finally {
