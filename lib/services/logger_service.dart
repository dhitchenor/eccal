...
class LoggerService {
  ...
  void setLogLevel(LogLevel level) {
    ...
    if (level != LogLevel.none && _initialized) {
      _writeLog(LogLevel.info, 'Log level changed from ${oldLevel.name} to ${level.name}');
    }
  }
  ...
  Future<String?> exportLogs(String outputDirectory) async {
    try {
      ...
      final exportFile = File('$outputDirectory/eccal_logs_$timestamp.txt');
