            ...
            if ($pubspecLine) {
                ...
                if ($pubspecVersion -and ($pubspecVersion -ne $configVersion)) {
                    ...
                    if (Compare-Versions $pubspecVersion $configVersion) {
                        Write-Host ""
                        Write-Host "⚠️  Dependency version difference detected:" -ForegroundColor Yellow
        ...
        if ($_ -match "'([^']+)':\s*'([^']+)'") {
            ...
            if ($pubspecLine) {
                ...
                if ($pubspecVersion -and ($pubspecVersion -ne $configVersion)) {
                    ...
                    if (Compare-Versions $pubspecVersion $configVersion) {
                        Write-Host ""
                        Write-Host "⚠️  DevDependency version difference detected:" -ForegroundColor Yellow
                        Write-Host "    Package: $package" -ForegroundColor White
                        Write-Host "    app_config.dart: $configVersion" -ForegroundColor White
                        Write-Host "    pubspec.yaml:    $pubspecVersion" -ForegroundColor White
                        Write-Host ""
...
Write-Host ""
Write-Host "ℹ️  versionCode is used by Google Play to identify app versions." -ForegroundColor Cyan
