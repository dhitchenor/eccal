...
class DateFormatter {
  ...
  // Helper method to convert UTC time to specified timezone
  static DateTime _convertToTimezone(DateTime utcTime, String? timezone) {
    // If no timezone specified, return as-is (assume local)
    if (timezone == null) {
      return utcTime;
    }
    ...
    // Parse offset from string like "+1000" or "-0500"
    final offsetString = tzData.standardOffset;
    final isNegative = offsetString.startsWith('-');
    ...
    // Assuming utcTime is stored in UTC
    final offset = Duration(
      hours: isNegative ? -offsetHours : offsetHours,
      minutes: isNegative ? -offsetMinutes : offsetMinutes,
    );
