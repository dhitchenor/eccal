...
class ICalendarGenerator {
  ...
  static String generate(DiaryEntry entry, {int durationMinutes = 15}) {
    ...
    if (timezone == 'UTC') {
      ...
      buffer.writeln('DTSTART;TZID=$timezone:${formatICalDateTime(entry.dtstart, useLocal: true)}');
    }
    ...
    for (int i = 0; i < entry.appendDates.length; i++) {
      ...
      if (timezone == 'UTC') {
        // For UTC timezone, use Z suffix
        buffer.writeln('X-APPENDDATE;INDEX=$appendIndex:${formatICalDateTime(entry.appendDates[i])}');
      } else {
        // For non-UTC timezones, use TZID parameter
        buffer.writeln('X-APPENDDATE;INDEX=$appendIndex;TZID=$timezone:${formatICalDateTime(entry.appendDates[i], useLocal: true)}');
      }
      
      // X-APPENDMOOD for this append (use index to match)
      if (i < entry.appendMoods.length) {
        buffer.writeln('X-APPENDMOOD;INDEX=$appendIndex:${entry.appendMoods[i]}');
      } else {
        buffer.writeln('X-APPENDMOOD;INDEX=$appendIndex:neutral'); // Default if missing
      }
      ...
      if (i < entry.appendLocations.length && entry.appendLocations[i].isNotEmpty) {
        buffer.writeln('X-APPENDLOC;INDEX=$appendIndex:${entry.appendLocations[i]}');
      }
      ...
      if (i < entry.appendLatitudes.length && 
        ...
        buffer.writeln('X-APPENDGEO;INDEX=$appendIndex:${entry.appendLatitudes[i]};${entry.appendLongitudes[i]}');
      }
    }
