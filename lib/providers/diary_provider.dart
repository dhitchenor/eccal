...
class DiaryProvider with ChangeNotifier {
  ...
  // CalDAV settings - will be set from SettingsProvider
  CalDAVService? _caldavService;
  String? _caldavCalendarName;
  int _eventDurationMinutes = 15;
  
  // Settings provider reference for file format
  ...
  // Sync with CalDAV - download entries and merge with local
  Future<void> syncWithCalDAV() async {
    ...
    logger.info('Starting CalDAV sync');
    ...
    try {
      ...
      final calendarEvents = await _caldavService!.fetchEvents(
        calendarName: _caldavCalendarName,
      );
      ...
      for (final event in calendarEvents) {
        ...
        final existingEntry = _entries.where((e) => e.id == entryId).firstOrNull;
        ...
        if (existingEntry == null) {
          ...
          final entry = DiaryEntry(
            ...
            appendLatitudes: [],
            appendLongitudes: [],
            timezone: _settingsProvider?.timezone ?? 'UTC',
          );
          
          caldavEntries.add(entry);
          
          // Save to local file storage
          final fileFormat = _settingsProvider?.fileFormat == FileFormat.txt ? 'txt' : 'ics';
          await _fileStorage.saveEntry(entry, fileFormat: fileFormat, timezone: entry.timezone);
        } else {
          // Entry exists locally - check if server version is newer
          final serverModified = event.dtstamp ?? event.dtstart;
          final localModified = existingEntry.dtstamp ?? existingEntry.dtstart;
          
          if (serverModified.isAfter(localModified)) {
            // Server version is newer - update local entry
            logger.info('Server version of "${event.summary}" is newer, updating local copy');
            final updatedEntry = DiaryEntry(
              ...
              appendDates: existingEntry.appendDates,
              appendMoods: existingEntry.appendMoods,
              appendLocations: existingEntry.appendLocations,
              appendLatitudes: existingEntry.appendLatitudes,
              appendLongitudes: existingEntry.appendLongitudes,
            ...
            // Save to local file storage
            final fileFormat = _settingsProvider?.fileFormat == FileFormat.txt ? 'txt' : 'ics';
            await _fileStorage.saveEntry(updatedEntry, fileFormat: fileFormat, timezone: updatedEntry.timezone);
      ...
      final serverEventIds = calendarEvents.map((e) => e.uid.split('@')[0]).toSet();
      int uploadedCount = 0;
      ...
      for (final localEntry in _entries) {
        if (!serverEventIds.contains(localEntry.id)) {
          ...
          try {
            await _caldavService!.createEvent(
              entry: localEntry,
            ...
            logger.error('Error uploading entry ${localEntry.id} to server: $e');
  ...
  // Check if an entry exists on CalDAV server
  ...
  Future<Map<String, bool>> getServerStatusForAllEntries() async {
    ...
    if (!isCalDAVConfigured || (_settingsProvider?.caldavSyncDisabled ?? false)) {
  ...
  void setSettingsProvider(SettingsProvider provider) {
    ...
    if (provider.serverPollIntervalMinutes != null) {
      startServerPolling();
    }
  }
  ...
  void configureCalDAV({
    ...
    if (url != null && url.isNotEmpty && 
        ...
        calendarName != null && calendarName.isNotEmpty) {
      ...
      logger.info('CalDAV configured: $url, calendar: $calendarName');
    } else {
      _caldavService = null;
      _caldavCalendarName = null;
      logger.info('CalDAV disabled (incomplete settings)');
    }
  }

  // Check if CalDAV is configured
  bool get isCalDAVConfigured => _caldavService != null && _caldavCalendarName != null;
  ...
  Future<void> addEntry(
    ...
    try {
      ...
      await Future.delayed(const Duration(milliseconds: 300)); // Small delay so user can see the message
      final fileFormat = _settingsProvider?.fileFormat == FileFormat.txt ? 'txt' : 'ics';
    ...
    if (isCalDAVConfigured && !(_settingsProvider?.caldavSyncDisabled ?? false)) {
      try {
        ...
        logger.info('Syncing entry to CalDAV: ${entry.title}');
        await _caldavService!.createEvent(
          entry: entry,
          durationMinutes: _eventDurationMinutes,
          calendarName: _caldavCalendarName!,
        );
        logger.info('Entry synced to CalDAV successfully: ${entry.title}');
        // Check server status for this entry only
        await checkServerStatusForEntry(entry.id);
      } catch (e) {
        logger.error('Error syncing entry to CalDAV: $e');
  ...
  Future<void> updateEntry(
    ...
    if (index != -1) {
      ...
      try {
        onProgress?.call('save_dialog.saving_local'.tr());
        await Future.delayed(const Duration(milliseconds: 500)); // Small delay so user can see the message
        final fileFormat = _settingsProvider?.fileFormat == FileFormat.txt ? 'txt' : 'ics';
      ...
      if (isCalDAVConfigured && !(_settingsProvider?.caldavSyncDisabled ?? false)) {
        try {
          ...
          await _caldavService!.updateEvent(
            entry: updatedEntry,
            durationMinutes: _eventDurationMinutes,
            calendarName: _caldavCalendarName!,
          );
          logger.info('Entry updated in CalDAV successfully: ${updatedEntry.title}');
          // Check server status for this entry only
          await checkServerStatusForEntry(updatedEntry.id);
        } catch (e) {
          logger.error('Error updating entry in CalDAV: $e');
          // Don't rethrow - local save succeeded, server sync failed is ok
        }
      } else {
        if (_settingsProvider?.caldavSyncDisabled ?? false) {
  ...
  Future<void> deleteEntry(String entryId) async {
    ...
    if (isCalDAVConfigured && !(_settingsProvider?.caldavSyncDisabled ?? false)) {
      try {
        logger.info('Deleting entry from CalDAV: $entryId');
        await _caldavService!.deleteEvent(
          entryId,
          _caldavCalendarName!,
        );
        logger.info('Entry deleted from CalDAV successfully: $entryId');
      } catch (e) {
        logger.error('Error deleting entry from CalDAV: $e');
      }
    } else {
      if (_settingsProvider?.caldavSyncDisabled ?? false) {
        logger.info('CalDAV sync disabled, skipping sync');
  ...
  Future<void> deleteServerOnly(String entryId) async {
    ...
    if (isCalDAVConfigured && !(_settingsProvider?.caldavSyncDisabled ?? false)) {
      try {
        logger.info('Deleting entry from CalDAV only: $entryId');
        await _caldavService!.deleteEvent(
          entryId,
          _caldavCalendarName!,
        );
  ...
  Future<void> resetToDefaultPath() async {
    try {
      ...
      for (final entry in currentEntries) {
        await _fileStorage.saveEntry(entry, fileFormat: fileFormat, timezone: timezone);
      }
