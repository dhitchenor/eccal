    ...
    try {
      ...
      await _timedStep('setup.checking_provider'.tr(), () async {
        ...
        if (provider == CalendarProvider.caldav) {
          ...
          isProviderAuthenticated = widget.settingsProvider.caldavUrl != null &&
          ...
          if (isProviderAuthenticated) {
            widget.diaryProvider.configureCalDAV(
              ...
              eventDurationMinutes: widget.settingsProvider.eventDurationMinutes,
            );
          ...
          isProviderAuthenticated = widget.settingsProvider.googleCalendarId != null &&
          ...
          if (isProviderAuthenticated) {
            ...
            // Preload calendar list for fast server tab loading
            try {
              final calendars = await widget.diaryProvider.googleCalendarService.listCalendars();
              if (calendars.isNotEmpty) {
                await widget.settingsProvider.setCachedCalendarList(calendars);
      ...
      // Step 8: Perform initial server sync (ONLY if authenticated and sync enabled)
      final shouldSync = isProviderAuthenticated &&
                        !widget.settingsProvider.caldavSyncDisabled &&
                        widget.diaryProvider.isCalDAVConfigured;

      if (shouldSync) {
        await _timedStep('setup.syncing_server'.tr(), () async {
          try {
            await widget.diaryProvider.syncWithCalDAV();
