...
class EntryConverter {
  ...
  static String _applyInlineFormatting(String text, Map<String, dynamic> attributes) {
...
```''';
  ...
  static Document fromMarkdown(String markdown) {
    ...
    for (int lineIdx = 0; lineIdx < lines.length; lineIdx++) {
      ...
      if (line.startsWith('###### ')) {
        line = line.substring(7).replaceAll(RegExp(r'\s*######\s*$'), '').trim();
  ...
  // Insert a formatted line with line-level attribute
  static int _insertFormattedLine(Document doc, int offset, String text, Attribute lineAttribute) {
    final startOffset = offset;
    
    // Always apply inline formatting, even for headers
    offset = _insertLineWithInlineFormatting(doc, offset, text);
    
    doc.insert(offset, '\n');
    // Format from start of line through the newline
    doc.format(startOffset, offset - startOffset + 1, lineAttribute);
    return offset + 1;
  }
  
  // Insert text with inline formatting (bold, italic, etc.)
  // IMPORTANT: We need to ensure attributes don't bleed between segments
  static int _insertLineWithInlineFormatting(Document doc, int offset, String text) {
    final segments = _parseInlineMarkdown(text);
    
    for (int i = 0; i < segments.length; i++) {
      final segment = segments[i];
      if (segment.text.isNotEmpty) {
        // Insert text  
        doc.insert(offset, segment.text);
        
        // Apply attributes if any
        if (segment.attributes.isNotEmpty) {
          for (final attr in segment.attributes) {
            doc.format(offset, segment.text.length, attr);
          }
        }
  ...
  static List<_StyledText> _parseWithPriority(String text, int skipLevel) {
    ...
    while (i < text.length) {
      ...
      if (skipLevel < 1 && i + 2 < text.length && text.substring(i, i + 3) == '***') {
        ...
        if (end != -1) {
          ...
          final innerSegments = _parseWithPriority(text.substring(i + 3, end), 1);
          for (final segment in innerSegments) {
            result.add(_StyledText(segment.text, [Attribute.bold, Attribute.italic, ...segment.attributes]));
          }
          i = end + 3;
          matched = true;
        }
      ...
      if (!matched && skipLevel < 1 && i + 1 < text.length && text.substring(i, i + 2) == '**') {
        ...
        if (end != -1) {
          ...
          // Recursively parse inner content with italic+HTML priority
          final innerSegments = _parseWithPriority(text.substring(i + 2, end), 1);
          for (final segment in innerSegments) {
            result.add(_StyledText(segment.text, [Attribute.bold, ...segment.attributes]));
          }
          i = end + 2;
          matched = true;
        }
      ...
      if (!matched && skipLevel < 2 && (text[i] == '*' || text[i] == '_')) {
        ...
        if (end != -1 && end > i + 1) {
          ...
          final innerSegments = _parseWithPriority(text.substring(i + 1, end), 2);
          for (final segment in innerSegments) {
            result.add(_StyledText(segment.text, [Attribute.italic, ...segment.attributes]));
          }
          i = end + 1;
          matched = true;
        }
      ...
      if (!matched && i + 1 < text.length && text.substring(i, i + 2) == '~~') {
        ...
        if (end != -1) {
          ...
          result.add(_StyledText(text.substring(i + 2, end), [Attribute.strikeThrough]));
          i = end + 2;
          matched = true;
        }
      }
      ...
      if (!matched && text[i] == '`') {
        ...
        if (end != -1) {
          ...
          result.add(_StyledText(text.substring(i + 1, end), [Attribute.inlineCode]));
          i = end + 1;
          matched = true;
        }
      }
      ...
      if (!matched && i + 2 < text.length && text.substring(i, i + 3) == '<u>') {
        ...
        if (end != -1) {
          ...
          final innerSegments = _parseWithPriority(text.substring(i + 3, end), 3);
          for (final segment in innerSegments) {
            result.add(_StyledText(segment.text, [Attribute.underline, ...segment.attributes]));
          }
          i = end + 4;
          matched = true;
        }
      ...
      if (!matched && i + 4 < text.length && text.substring(i, i + 5) == '<sub>') {
        ...
        if (end != -1) {
          ...
          // Recursively parse inner content to handle nested tags
          final innerSegments = _parseWithPriority(text.substring(i + 5, end), 3);
          for (final segment in innerSegments) {
            result.add(_StyledText(segment.text, [Attribute.subscript, ...segment.attributes]));
          }
          i = end + 6;
          matched = true;
        }
      ...
      if (!matched && i + 4 < text.length && text.substring(i, i + 5) == '<sup>') {
        ...
        if (end != -1) {
          ...
          final innerSegments = _parseWithPriority(text.substring(i + 5, end), 3);
          for (final segment in innerSegments) {
            result.add(_StyledText(segment.text, [Attribute.superscript, ...segment.attributes]));
          }
          i = end + 6;
          matched = true;
        }
