...
class _CalendarSignInWidgetState extends State<CalendarSignInWidget> {
        ...
        return Column(
          ...
          children: [
            ...
            ListTile(
              ...
              title: Text(
                'server_settings.signed_in_as'.tr([_getUserIdentifier(settings)]),
              ),
              subtitle: Text(_getProviderName()),
              trailing: TextButton(
                onPressed: widget.syncDisabled ? null : () => _handleSignOut(context, settings),
            ...
            if (snapshot.connectionState == ConnectionState.waiting)
              ...
              Padding(
                padding: const EdgeInsets.all(16.0),
                child: _buildCalendarComboBox(snapshot.data!, settings, context),
              ),
          ],
        );
      },
  ...
  String _getUserIdentifier(SettingsProvider settings) {
    switch (widget.provider) {
      ...
      case CalendarProvider.apple:
        return 'Apple User'; // TODO: Get from Apple service
    }
  }
  ...
  Future<List<CalendarInfo>> _fetchCalendarsFromServer(SettingsProvider settings) async {
    List<CalendarInfo> calendars;

    switch (widget.provider) {
      case CalendarProvider.google:
        if (widget.googleService == null) throw Exception('Google service not provided');
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            return DropdownMenu<CalendarInfo>(
              width: finalWidth,
              menuHeight: 300, // Limit height to show ~5 entries (60px per entry)
  ...
  String? _getCurrentCalendarId(SettingsProvider settings) {
    switch (widget.provider) {
      ...
      case CalendarProvider.apple:
        return null; // TODO: Apple calendar ID
    }
  }
  ...
  Future<String> _createCalendar(String calendarName, SettingsProvider settings) async {
    switch (widget.provider) {
      case CalendarProvider.google:
        if (widget.googleService == null) throw Exception('Google service not available');
      ...
      case CalendarProvider.apple:
        throw UnimplementedError('Apple Calendar not yet implemented');
    }
  }

  Future<void> _saveCalendarSelection(String calendarId, SettingsProvider settings) async {
    switch (widget.provider) {
      case CalendarProvider.google:
  ...
  Future<void> _refreshDiaryProvider(
    ...
    switch (widget.provider) {
      ...
      case CalendarProvider.apple:
        // TODO: Apple calendar configuration
        break;
    }
  }

  Future<void> _handleSignOut(BuildContext context, SettingsProvider settings) async {
    switch (widget.provider) {
      case CalendarProvider.google:
