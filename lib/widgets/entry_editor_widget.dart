...
class EntryEditorWidgetState extends State<EntryEditorWidget> {
  ...
  final GlobalKey<SimpleRichTextEditorState> _editorKey = GlobalKey<SimpleRichTextEditorState>();
            ...
            child: Row(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(widget.isAppendMode ? 'editor.save_append'.tr() : 'editor.save_entry'.tr()),
                ),
              ],
            ),
          ),
    ...
    try {
      ...
      if (!serviceEnabled) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('editor.location_services_disabled'.tr())),
          );
        }
        return;
      }
      ...
      if (permission == LocationPermission.denied) {
        ...
        if (permission == LocationPermission.denied) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('editor.location_permission_denied'.tr())),
            );
          }
          return;
        }
      ...
      if (permission == LocationPermission.deniedForever) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('editor.location_permission_permanently_denied'.tr())),
          );
        }
        return;
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('editor.getting_gps_location'.tr()), duration: const Duration(seconds: 2)),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('editor.location_added'.tr([coordinates]))),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('editor.error_getting_location'.tr([e.toString()]))),
        );
      }
    }
  }
  ...
  void _handleSave() {
    ...
    final title = widget.isAppendMode ? widget.entry!.title : _titleController.text;
